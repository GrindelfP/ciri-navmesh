...
VertexIdx DCEL::splitFace(FaceIdx fi, Point2D p) {
    ...
    // ── f1: a → b → p ────────────────────────────────────────────────────
    halfEdges_[e0    ].next = bpInner; halfEdges_[e0    ].prev = paInner; halfEdges_[e0    ].face = f1;
    halfEdges_[bpInner].next = paInner; halfEdges_[bpInner].prev = e0;     halfEdges_[bpInner].face = f1;
    halfEdges_[paInner].next = e0;     halfEdges_[paInner].prev = bpInner; halfEdges_[paInner].face = f1;
    faces_[f1].edge = e0;
    faces_[f1].dead = false;

    // ── f2: b → c → p ────────────────────────────────────────────────────
    halfEdges_[e1    ].next = cpInner; halfEdges_[e1    ].prev = pbInner; halfEdges_[e1    ].face = f2;
    halfEdges_[cpInner].next = pbInner; halfEdges_[cpInner].prev = e1;     halfEdges_[cpInner].face = f2;
    halfEdges_[pbInner].next = e1;     halfEdges_[pbInner].prev = cpInner; halfEdges_[pbInner].face = f2;
    faces_[f2].edge = e1;

    // ── f3: c → a → p ────────────────────────────────────────────────────
    halfEdges_[e2    ].next = apInner; halfEdges_[e2    ].prev = pcInner; halfEdges_[e2    ].face = f3;
    halfEdges_[apInner].next = pcInner; halfEdges_[apInner].prev = e2;     halfEdges_[apInner].face = f3;
    halfEdges_[pcInner].next = e2;     halfEdges_[pcInner].prev = apInner; halfEdges_[pcInner].face = f3;
    faces_[f3].edge = e2;
...
VertexIdx DCEL::splitEdge(HalfEdgeIdx h, Point2D p) {
    ...
    halfEdges_[h ].twin = upInner;  halfEdges_[h ].origin = u;
    halfEdges_[ht].twin = vpInner;  halfEdges_[ht].origin = v;
    ...
    // fLeft: u → p → W → u
    halfEdges_[h      ].next = pwInner; halfEdges_[h      ].prev = hp;      halfEdges_[h      ].face = fLeft;
    halfEdges_[pwInner].next = hp;      halfEdges_[pwInner].prev = h;       halfEdges_[pwInner].face = fLeft;
    halfEdges_[hp     ].next = h;       halfEdges_[hp     ].prev = pwInner; halfEdges_[hp     ].face = fLeft;
    faces_[fLeft].edge = h;
    faces_[fLeft].dead = false;

    // fLeftV: p → v → W → p
    halfEdges_[pvInner].next = hn;      halfEdges_[pvInner].prev = wpInner; halfEdges_[pvInner].face = fLeftV;
    halfEdges_[hn     ].next = wpInner; halfEdges_[hn     ].prev = pvInner; halfEdges_[hn     ].face = fLeftV;
    halfEdges_[wpInner].next = pvInner; halfEdges_[wpInner].prev = hn;      halfEdges_[wpInner].face = fLeftV;
    ...
    // fRight: v → p → X → v
    halfEdges_[ht     ].next = pxInner; halfEdges_[ht     ].prev = htp;     halfEdges_[ht     ].face = fRight;
    halfEdges_[pxInner].next = htp;     halfEdges_[pxInner].prev = ht;      halfEdges_[pxInner].face = fRight;
    halfEdges_[htp    ].next = ht;      halfEdges_[htp    ].prev = pxInner; halfEdges_[htp    ].face = fRight;
    faces_[fRight].edge = ht;
    faces_[fRight].dead = false;

    // fRightV: p → u → X → p  (htn is u→X)
    halfEdges_[puInner].next = htn;     halfEdges_[puInner].prev = xpInner; halfEdges_[puInner].face = fRightV;
    halfEdges_[htn    ].next = xpInner; halfEdges_[htn    ].prev = puInner; halfEdges_[htn    ].face = fRightV;
    halfEdges_[xpInner].next = puInner; halfEdges_[xpInner].prev = htn;     halfEdges_[xpInner].face = fRightV;
...
void DCEL::removeVertex(VertexIdx vi) {
    assertVertex(vi);
    // Mark all incident faces dead.
    for (HalfEdgeIdx he : vertexStar(vi)) {
        const FaceIdx fi = halfEdges_[he].face;
        if (fi != kOuterFace && !faces_[fi].dead)
            killFace(fi);
        halfEdges_[he].dead = true;
        halfEdges_[halfEdges_[he].twin].dead = true;
    }
