...
namespace geometry::predicates {
    ...
    IntersectionType segmentIntersect(const Point2D &p1,
        ...
        if (allCollinear) {
            // Degenerated segment cannot give Overlap — only Endpoint.
    ...
    bool lineIntersectionPoint(const Point2D &p1,
                               ...
                               Point2D &out,
                               double eps) noexcept {
        // Parametric: P = p1 + t*(p2-p1), Q = p3 + u*(p4-p3).
        // P = Q:
    ...
    double signedAngle(const Point2D &a,
        ...
        // atan2(cross, dot) — signed angle from ab to ac.
