...
Orientation orientation(const Point2D& a,
                        ...
                        double eps) noexcept {
    // det = (b-a) × (c-a)
    // Разбиваем на два шага, чтобы минимизировать потери точности:
    //   используем компенсацию Кэхана при необходимости... или просто
    //   аккуратную двойную арифметику с масштабированным eps.
    const double det = (b - a).cross(c - a);

    // Масштабируем порог на характерный размер треугольника,
    // чтобы eps работал независимо от масштаба координат.
    // Используем сумму длин сторон как нормировочный множитель.
    // Подход консервативен: при больших треугольниках порог мягче.
...
CirclePosition inCircle(const Point2D& a,
                        ...
                        const Point2D& d,
                        double eps) noexcept {
    // Сдвигаем систему отсчёта в точку d — повышает численную стабильность.
    ...
    //       | cx  cy  cx²+cy² |
    //
    // Раскладываем по третьему столбцу (cofactor expansion):
    ...
    // Масштабируем порог на объём матрицы (максимум элементов).
    // Это стандартная практика для численных предикатов.
    ...
    // Для CCW-треугольника (a,b,c): det > 0 ↔ d внутри.
...
bool onSegment(const Point2D& a,
               ...
               const Point2D& p,
               double eps) noexcept {
    // Предполагаем коллинеарность (проверена вызывающим кодом).
    // Bbox-тест: p лежит в прямоугольнике, ограничивающем [a, b].
...
IntersectionType segmentIntersect(const Point2D& p1,
                                   ...
                                   const Point2D& p4,
                                   double eps) noexcept {
    // Алгоритм Shamos–Hoey с обработкой граничных случаев.
    //
    // Ориентации точек p3, p4 относительно прямой p1-p2:
    const Orientation d1 = orientation(p1, p2, p3, eps);
    const Orientation d2 = orientation(p1, p2, p4, eps);
    // Ориентации точек p1, p2 относительно прямой p3-p4:
